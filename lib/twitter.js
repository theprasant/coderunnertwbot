...
import twitterParser from 'twitter-text';
  ...
  for (let mention of mentionsArr) {
    ...
    try {
      ...
      if ((new RegExp(`@${config.username}\\s+render\\b`, 'i')).test(mention.text)) {
        ...
        command = 'run';
      } else if ((new RegExp(`@${config.username}\\s+mdn\\b`, 'i')).test(mention.text)) {
        command = 'mdn';
      } else {
        continue;
      }
      ...
      switch (command) {
        case 'render':
          ...
          try {
            ...
            statusText += `@${authorUser.data.username} page: https://coderunnerbot.vercel.app/page/${post.author}/${slug}\ncode: https://coderunnerbot.vercel.app/code/${post.author}/${slug}`;
        ...
        case 'mdn':
          try {
            let mdnReg = new RegExp(`(?<=@${config.username}(\n+| +| +\n+|\n+ +| +\n+ +)mdn)[^\n]+`, 'i');
            let q = mention.text.match(mdnReg);
            if (!q) continue;
            let mdnUrl = `https://developer.mozilla.org/api/v1/search?q=${encodeURIComponent(q[0])}`;
            let mdnRes = await fetch(mdnUrl);
            let mdnData = await mdnRes.json();
            let mdnResults = mdnData.documents;
            if (!mdnResults || mdnResults.length < 1) {
              statusText = `@${authorUser.data.username} No results found for ${q[0]}`.substring(0, 280 - authorUser.data.username.length - 40);
            }else{
              statusText = `@${authorUser.data.username} details: https://developer.mozilla.org${mdnResults[0].mdn_url}\n${mdnResults[0].title}\n\n${mdnResults[0].summary.replace(/\n/g,'')}`.substring(0, 280 - authorUser.data.username.length - 10);
            }
          } catch (error) {
            console.error(error);
          }
