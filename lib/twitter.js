      ...
      switch (command) {
        ...
        case 'run':
          try {
            ...
            //   statusText += '...';
            // }
            // let mentionsText = pTweet?.entities?.user_mentions?.map(user => `@${user.screen_name}`).join(' ');
            mentionsText = '';
            let detailsPageText = `\ndetails: https://coderunnerbot.vercel.app/output/${mention.author_id}/${codeSlug}`;
            statusText = statusText.substring(0, 280 - (mentionsText?mentionsText.length:0) - 3 - authorUser.data.username.length - 5 - detailsPageText.length - 20 );
