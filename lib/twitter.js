...
const replyMentionedTweets = async (mentionsArr, { statusText, statusTextFormatter }) => {
  ...
  for (let mention of mentionsArr) {
    ...
    try {
      ...
      //   const newPost = await postRef.set(post);
      //   statusText += `page: https://coderunnerbot.vercel.app/page/${post.author}/${slug}\ncode: https://coderunnerbot.vercel.app/code/${post.author}/${slug}`;
      //   // loadMsg.edit(`slug: ${slug}\n> page: https://coderunnerbot.vercel.app/page/${message.author.id}/${slug}\n> code: https://coderunnerbot.vercel.app/code/${message.author.id}/${slug}${reDirectMsg}${scriptMsg}\n> Requested by: ${message.author.tag} (${message.author.id} | ${message.author})`);
      //   // if (args[0].toLowerCase() == "-p") loadMsg.reply(`https://s.vercel.app/api?url=https://coderunnerbot.vercel.app/page/${message.author.id}/${slug}&width=1280&height=720`)
      //   console.log('statusText: ', statusText);
